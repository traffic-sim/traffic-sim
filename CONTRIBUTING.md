...
This project uses Linear for issue tracking and GitHub for version control and collaboration.
...
2. Create a branch: `ts-<number>-short-description`
3. Commit using:

   ```
   <type>: <short description> (TS-XX)
   ```
...
* Every change must be linked to a Linear issue
...
Prefer using branch names generated by Linear.
...
* `feat: add density update (TS-12)`
* `fix: correct simulation step logic (TS-14)`
...
* Every PR must be linked to a Linear issue
